...
try:
    from .mamba2 import selective_scan_chunk_fn
except ImportError:
    from mamba2 import selective_scan_chunk_fn
    ...
    def __initv2__(
        ...
        **kwargs,
        ...
        FORWARD_TYPES = dict(
            ...
            v05=partial(
                self.forward_corev2, force_fp32=False, no_einsum=True
        ...
        **kwargs,
        ...
        if _scan_mode == 3:
            ...
            if getattr(self, '__DEBUG__', False):
                self.__data__ = dict(
                    A_logs=self.A_logs, Bs=Bs, Cs=Cs, Ds=Ds, us=xs, dts=dts, delta_bias=delta_bias, ys=ys, y=y, H=H, W=W
                )
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        us, dts, Bs, Cs = x.split(
            [self.d_inner, self.dts_dim, 4 * self.d_state, 4 * self.d_state], dim=(1 if self.channel_first else -1)
        )
        ...
        Bs = cross_scan_fn(
            Bs.contiguous(), in_channel_first=self.channel_first, out_channel_first=True, one_by_one=True
        ).view(B, 4, -1, L)
        Cs = cross_scan_fn(
            Cs.contiguous(), in_channel_first=self.channel_first, out_channel_first=True, one_by_one=True
        ).view(B, 4, -1, L)
        dts = cross_scan_fn(
            dts.contiguous(),
            in_channel_first=self.channel_first,
            out_channel_first=True,
            one_by_one=(self.dts_dim == 4 * self.dt_rank),
        ).view(B, L, -1)
        if self.dts_dim == self.dt_rank or self.dts_dim == 4 * self.dt_rank:
        ...
        if getattr(self, '__DEBUG__', False):
            self.__data__ = dict(
                A_logs=self.A_logs, Bs=Bs, Cs=Cs, Ds=Ds, us=us, dts=dts, delta_bias=delta_bias, ys=ys, y=y
            )
...
class VSSM(nn.Module):
    ...
    def _make_layer(
        ...
        downsample=nn.Identity(),  # noqa: B008
        channel_first=False,
        # ===========================
        ssm_d_state=16,
        ssm_ratio=2.0,
